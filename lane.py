...
class Lane():
    def __init__(self,orig_frame):
        ...
        # (Width, Height) of the origal video frame (or image)
        ...
        self.roi_points = np.float32([
          (40,280), # Top-left corner
          (10, 470), # Bottom-left corner            
          (600,470), # Bottom-right corner
          (630,280) # Top-right corner
        ])
    ...
    def display_curvature_offset(self, frame=None, plot=False):
        ...
        cv2.putText(image_copy,'Curve Radius: '+str((
          self.left_curvem+self.right_curvem)/2)[:7]+' m',
                    (int((5/640)*self.width),int((20/480)*self.height)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    (float((0.5/640)*self.width)),
                    (255,255,255),2,cv2.LINE_AA)
        cv2.putText(image_copy,'Center Offset: '+str(
          self.center_offset)[:7]+' cm',
                    (int((5/640)*self.width), int((40/480)*self.height)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    (float((0.5/640)*self.width)),
                    (255,255,255),2,cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not ret:
            print("cannnot read camera")
        lane_obj = Lane(orig_frame=original_frame)
        lane_line_marking = lane_obj.get_line_markings()
